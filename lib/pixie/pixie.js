(function() {
  ...
  Editor.prototype.addVariable = function(name, local, cloud) {
    name = name.trim();
    if (!name) return;
    if (this.checkName(name, local, T('New Variable'))) return;
  ...
  Editor.prototype.addList = function(name, local, cloud) {
    name = name.trim();
    if (!name) return;
    if (this.checkName(name, local, T('New List'))) return;
  ...
  Editor.prototype.checkName = function(name, local, title) {
    ...
    if (old) {
      Dialog.alert(title, T(old.isList ? 'A list with the name “{name}” already exists.' : 'A variable with the name “{name}” already exists.', {name: name})).show(this); // NS
    }
    return old;
  };
