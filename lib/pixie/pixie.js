(function() {
  ...
  function Watcher(target, kind, arg) {
    ...
    this.color = Watcher.colors[kind] || Watcher.colors.undefined;
    this.isSensor = Watcher.sensors.indexOf(kind) !== -1;

    if (kind === 'readVariable') {
      this.variable = target.findLocal(arg);
      if (!this.variable) kind = 'undefined';
    }
    this.read = Watcher.readFns[kind] || Watcher.readFns.undefined;

    this.el = el('watcher');
    this.el.appendChild(this.elLabel = el('watcher-label'));
    this.el.appendChild(this.elContents = el('watcher-contents'));
  ...
  Watcher.sensors = ['timer', 'soundLevel', 'timeAndDate', 'senseVideoMotion'];
  Watcher.globals = ['answer', 'timer', 'soundLevel', 'timeAndDate', 'backgroundIndex', 'tempo', 'sceneName'];
  ...
  Watcher.prototype.updateLabel = function() {
    var label =
      this.kind === 'readVariable' ? this.arg :
      this.kind === 'timeAndDate' ? T('current {time}', {time: this.arg}) :
      this.kind === 'senseVideoMotion' ? T('video {type}', {type: this.arg}) :
      Watcher.labels[this.kind];
  ...
  Watcher.colors = {
    answer: getCategoryColor(7),
    backgroundIndex: getCategoryColor(2),
    costumeIndex: getCategoryColor(2),
    readVariable: getCategoryColor(9),
    heading: getCategoryColor(1),
    scale: getCategoryColor(2),
    sceneName: getCategoryColor(2),
    senseVideoMotion: getCategoryColor(7),
    soundLevel: getCategoryColor(7),
    tempo: getCategoryColor(3),
    timeAndDate: getCategoryColor(7),
    timer: getCategoryColor(7),
    volume: getCategoryColor(3),
    xpos: getCategoryColor(1),
    ypos: getCategoryColor(1)
  };
  ...
  function ListWatcher(target, list, width, height) {
    ...
    this.el.appendChild(this.elEmpty = el('list-watcher-empty'));
    this.elEmpty.textContent = T('(empty)');
  ...
  Editor.prototype.toggleWatcher = function(array) {
    ...
    var target = Watcher.globals.indexOf(array[0]) !== -1 ? this.stage : this.selectedSprite;
  ...
  function SpritePanel(editor) {
    ...
    this.elNewGroup.appendChild(this.elNewGroupLabel = el('new-group-label'));
    this.elNewGroupLabel.textContent = T('New sprite:');
