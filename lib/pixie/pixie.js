(function() {
  ...
  Stage.prototype.findWatcher = function(kind, arg, target) {
    var children = this.children;
    for (var i = children.length; i--;) {
      var c = children[i];
  ...
  Stage.prototype.updateWatchers = function(sensorOnly) {
    ...
    for (var i = children.length; i--;) {
      var c = children[i];
      if (c.update && (!sensorOnly || c.isSensor)) c.update();
    }
  };
  ...
  function Watcher(target, kind, arg) {
    this.target = target;
    this.kind = kind;
    this.arg = arg;
    this._visible = true;
  ...
  Watcher.deserialize = function(stage, json) {
    ...
    return target && new Watcher(target, json.cmd === 'getVar:' ? 'readVariable' : json.cmd, json.param, json.color).fromJSON(json);
  };
  ...
  Watcher.kinds = {
    ...
    senseVideoMotion: {
      ...
      color: getCategoryColor(7),
      label: function() {
        return T('video {type}', {type: this.arg});
      }
    },
    soundLevel: {
      isSensor: true,
    ...
    timeAndDate: {
      ...
      color: getCategoryColor(7),
      label: function() {
        return T('current {time}', {time: this.arg});
      }
    },
    timer: {
      isSensor: true,
      ...
      label: 'timer',
      read: function() {
        return this.target.exec.getTimer().toFixed(1); // NS
      }
    },
    volume: {
      color: getCategoryColor(3),
  ...
  function ListWatcher(target, list, width, height) {
    ...
    this.el = cl('list-watcher', [
      ...
      this.elEmpty = cl('list-watcher-empty', [T('(empty)')]),
  ...
  function Editor() {
    ...
    this.el = cl('editor Visual-no-select', [
      this.elTopButtons = cl('project-buttons', [
        this.elShareButton = cl('button', 'project-button', [T('Share')]),
  ...
  Editor.prototype.hasWatcher = function(array) {
    var watcher = array[0] === 'contentsOfList:' ? this.findListWatcher(array[1]) : this.findWatcher(array);
  ...
  Editor.prototype.toggleWatcher = function(array) {
    ...
    var target = info && (info.isGlobal || info.isSensor) ? this.stage : this.selectedSprite;
    if (array[0] === 'readVariable' && !target.findLocal(array[1])) {
      target = this.stage;
      target.findOrCreateLocal(array[1]);
    }
    var watcher = this.stage.findWatcher(array[0], array[1], target);
    if (watcher) {
      watcher.visible = !watcher.visible;
      return watcher.visible;
    }
    watcher = new Watcher(target, array[0], array[1]);
  ...
  Editor.prototype.findListWatcher = function(name) {
    var target = this.selectedSprite;
    if (!target.findLocalList(name)) target = this.stage;
  ...
  Editor.prototype.findWatcher = function(array) {
    ...
    return this.stage.findWatcher(array[0], array[1], target);
  };
  ...
  ScriptsPanel.prototype.eval = function(t) {
    ...
    if (t.action) {
      ...
      var button = cl('button', 'ui-button', [T(t.text)]);
    ...
    if (t.text) {
      var div = cl('palette-label', [T(t.text)]);
  ...
  function CostumesPanel(editor) {
    ...
    this.el = cl('costumes-panel', [
      ...
      this.elNewGroup = cl('costume-new-group', [
        this.elLibraryButton = this.makeNewButton('new-library', T('Choose costume from library'), this.newFromLibrary),
        this.elPaintButton = this.makeNewButton('new-paint', T('Paint new costume'), this.newFromEditor),
        this.elImportButton = this.makeNewButton('new-import', T('Upload costume from file'), this.newFromFile, 'image/*'),
        this.elCameraButton = this.makeNewButton('new-camera', T('New costume from camera'), this.newFromCamera)
      ]),
  ...
  function TopBar(editor) {
    ...
    this.el = cl('top-bar', [
      ...
      this.fileButton = this.makeButton(T('File'), this.fileMenu, true),
      this.editButton = this.makeButton(T('Edit'), this.editMenu, true),
      this.tipsButton = this.makeButton(T('Tips'), this.showTips),
      this.aboutButton = this.makeButton(T('About'), this.showAbout),

      this.elTools = cl('top-tools', [
        this.duplicateButton = this.makeTool('duplicate', T('Duplicate')),
        this.deleteButton = this.makeTool('delete', T('Delete')),
        this.growButton = this.makeTool('grow', T('Grow')),
        this.shrinkButton = this.makeTool('shrink', T('Shrink')),
        this.helpButton = this.makeTool('help', T('Block help'))
      ])
    ]);
  ...
  function StagePanel(editor) {
    ...
    this.el.appendChild(this.elTitleBar = cl('title-bar'));
    ...
    this.elTitleBar.appendChild(this.elVersion = cl('version'));
    this.elVersion.textContent = editor.version;

    this.elTitleBar.appendChild(this.elTitle = cl('input', 'project-name'));
    this.elTitle.addEventListener('input', this.titleChanged.bind(this));
    this.updateTitle();

    this.elTitleBar.appendChild(this.elAuthor = cl('project-author'));
    this.updateAuthor();
    ...
    this.el.appendChild(this.elMouseCoords = cl('mouse-coords'));
    this.elMouseCoords.appendChild(this.elMouseXLabel = cl('mouse-label x-axis'));
    this.elMouseXLabel.textContent = 'x:';
    this.elMouseCoords.appendChild(this.elMouseX = cl('mouse-coord x-axis'));
    this.elMouseCoords.appendChild(this.elMouseXLabel = cl('mouse-label y-axis'));
    this.elMouseXLabel.textContent = 'y:';
    this.elMouseCoords.appendChild(this.elMouseY = cl('mouse-coord x-axis'));
  ...
  function SpritePanel(editor) {
    ...
    this.el = cl('sprite-panel', [
      this.elTitleBar = cl('title-bar', [
        this.elLabel = cl('title-label', [T('Sprites')]),
        this.elNewGroup = cl('new-group', [
          this.elNewGroupLabel = cl('new-group-label', [T('New sprite:')]),
          this.makeNewButton('new-library', T('Choose sprite from library'), this.newFromLibrary),
          this.makeNewButton('new-paint', T('Paint new sprite'), this.newFromEditor),
          this.makeNewButton('new-import', T('Upload sprite from file'), this.newFromFile, 'image/*,.sprite2'),
          this.makeNewButton('new-camera', T('New sprite from file'), this.newFromCamera)
        ])
      ]),
      this.elStageSection = cl('stage-section', [
        this.stageIcon.el,
        this.elNewBackdrop = cl('new-backdrop', [T('New backdrop:')])
      ]),
  ...
  function SpriteIcon(panel, sprite) {
    ...
    this.el.appendChild(this.elThumbnail = cl('canvas', 'sprite-thumbnail'));
    ...
    this.el.appendChild(this.elName = cl('sprite-icon-label'));
  ...
  function BackpackPanel(editor) {
    ...
    this.el.appendChild(this.elTitle = cl('backpack-title'));
    this.elTitle.textContent = 'Backpack';
    this.el.appendChild(this.elContent = cl('backpack-content'));
  ...
  function TipsPanel(editor) {
    ...
    this.el.appendChild(this.elTitle = cl('tips-title'));
    this.elTitle.appendChild(this.elIcon = cl('tips-icon'));
    this.elIcon.textContent = '?';
    this.elTitle.appendChild(this.elClose = cl('tips-close-button'));
    this.elClose.textContent = 'x';
    this.elTitle.appendChild(this.elLabel = cl('tips-label'));
    this.elLabel.textContent = T('Tips');
    this.elLabel.appendChild(this.elHome = cl('tips-home'));
    this.el.appendChild(this.elContent = cl('tips-content'));
    this.elContent.appendChild(this.elFrame = cl('iframe', 'tips-frame'));
  ...
  function Dialog(title, content) {
    ...
    this.el.appendChild(this.elTitle = cl('dialog-title'));
    this.el.appendChild(this.elContent = content || cl('dialog-content'));
  ...
  Dialog.Radio = function() {
    ...
    for (var i = 0, l = a.length; i < l; i++) {
      var s = a[i];
      var label = cl('label', 'dialog-radio-label enabled');
      var radio = cl('button', 'dialog-radio');
  ...
  Dialog.buttons = function() {
    ...
    for (var i = 0, l = a.length; i < l; i++) {
      ...
      var button = cl('button', 'ui-button');
