(function() {
  ...
  ScratchObj.prototype.updatePixi = function() {
    ...
    if (costume) {
      ...
      if (this.isSprite) {
        ...
        this.pixiSprite.scale.y = this.scale * costume.scale;
  ...
  function Costume(name, canvas, cx, cy, pixelRatio, textLayer) {
    ...
    if (typeof canvas === 'string') { // MD5
      ...
      this.baseLayerTexture = PIXI.Texture.fromImage(Server.getAssetURL(canvas), true, PIXI.scaleModes.LINEAR);
    } else {
      this.baseLayerTexture = PIXI.Texture.fromCanvas(canvas, PIXI.scaleModes.LINEAR);
    }
    this.baseLayerTexture.on('update', this.updateSize);
