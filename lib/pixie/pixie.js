(function() {
  ...
  function EffectsFilter() {
    ...
    '  if (mosaic != 1.) pos = mod(vTextureCoord * mosaic, 1.);',
    '  if (pixelSize != 1.) pos = (floor(vTextureCoord * dimensions.xy / pixelSize) + 0.5) / dimensions.xy * pixelSize;',
  ...
  ScratchObj.prototype.updatePixi = function() {
    ...
    if (this.effects.pixelate) {
      ...
      this.pixiEffectsFilter.pixelSize = Math.min(Math.abs(this.effects.pixelate / 10) + 1, costume.width, costume.height);
    } else {
      this.pixiEffectsFilter.pixelSize = 1;
    }
