(function() {
  ...
  ScratchObj.prototype.redraw = function() {
    ...
    if (this.costume !== this.renderedCostume) {
      this.renderedCostume = this.costume;
      var costume = this.costumes[this.costume];
      if (costume && costume.loaded) {
        this.canvas.width = costume.canvas.width;
        this.canvas.height = costume.canvas.height;
        this.canvas.style.transform = 'scale('+costume.scale+') translate('+(-costume.cx)+'px,'+(-costume.cy)+'px)';
        this.context.drawImage(costume.canvas, 0, 0);
      }
    }
    if (this.x !== this.renderedX || this.y !== this.renderedY || this.scale !== this.renderedScale || this.direction !== this.renderedDirection || this.rotationStyle !== this.renderedRotationStyle) {
      this.renderedX = this.x;
      this.renderedY = this.y;
      this.renderedScale = this.scale;
      this.renderedDirection = this.direction;
      this.renderedRotationStyle = this.rotationStyle;
      this.el.style.transform = 'translate('+(240 + this.x)+'px,'+(180 - this.y)+'px)'+(this.rotationStyle === 'normal' ? ' rotate('+(this.direction - 90)+'deg)' : this.rotationStyle === 'leftRight' && this.direction < 0 ? ' scale(-1, 1)' : '')+' scale('+this.scale+')';
    }
    if (this.effectsChanged) {
      this.el.style.opacity = 1 - this.effects.ghost / 100;
      var color = this.effects.color;
      var brightness = this.effects.brightness;
      this.el.style.filter =
      this.el.style.WebkitFilter =
        (color === 0 ? '' : 'hue-rotate('+(color * 1.8)+'deg)')+
        (brightness === 0 ? '' : 'brightness('+(brightness / 100 + 1)+')');
    }
  };
  ...
  Costume.prototype.changed = function(fromEditor) {
    if (this.owner) {
      ...
      if (i === this.owner.costume) {
        this.owner.renderedCostume = null;
        this.owner.redraw();
      }
  ...
  function Sprite(name) {
    ...
    this.renderedRotationStyle = null;

    this.el = cl('sprite Visual-absolute', [
  ...
  function Stage() {
    ...
    this.el = cl('stage', [
      this.canvas = el('canvas', {className: 'Visual-absolute'}),
      this.penCanvas = el('canvas', {className: 'Visual-absolute'})
    ]);
    this.context = this.canvas.getContext('2d');
  ...
  Stage.prototype.updatePixelRatio = function(quiet) {
    ...
    this.penCanvas.style.transform = 'scale('+(1/pr)+','+(1/pr)+')';
