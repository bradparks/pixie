(function() {
  ...
  function doFlip() {
    ...
    var esw = document.querySelector('.stage-wrapper');
    if (!flipped && !esw) {
      var esw = cl('stage-wrapper', [es]);
    ...
    esw.style.WebkitTransition = '-webkit-transform '+time+'s, z-index '+time+'s';
    if (flipped) {
      ed.style.WebkitTransform = 'none';
      es.style.WebkitTransform = 'none';
      esw.style.WebkitTransform = 'translateZ(0)';
    } else {
      ed.style.WebkitTransform = 'translate('+dx+'px,'+dy+'px) scale('+sx+','+sy+')';
      esw.style.WebkitTransform = 'translate3d('+(pbb.left-sbb.left)+'px,'+(pbb.top-sbb.top)+'px,0)';
    }
    flipped = !flipped;
  }
