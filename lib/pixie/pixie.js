(function() {
  ...
  function el(tagName, className, properties, children) {
  ...
  function Watcher(target, kind, arg) {
    ...
    this.el = el('watcher', [
      this.elLabel = el('watcher-label'),
      this.elContents = el('watcher-contents')
    ]);
  ...
  function ListWatcher(target, list, width, height) {
    ...
    this.el = el('list-watcher', [
      this.elTitle = el('list-watcher-title'),
      this.elContents = el('list-watcher-contents', [
        this.elFiller = el('list-watcher-filler')
      ]),
      this.elEmpty = el('list-watcher-empty', [T('(empty)')]),
      this.elLength = el('list-watcher-length'),
      this.elAddButton = el('button', 'list-watcher-add-button')
    ]);
  ...
  function ListCell(watcher, index, height, text) {
    ...
    this.el = el('list-cell', [
      this.elIndex = el('list-cell-index', [''+(index + 1)]),
      this.elContents = el('list-cell-contents', [text+'\u200B'])
    ]);
  ...
  ListCell.prototype.startEdit = function() {
    ...
    if (!this.elField) {
      this.el.appendChild(this.elField = el('textarea', 'list-cell-contents'));
  ...
  function Editor() {
    ...
    this.el = el('editor Visual-no-select', [
      this.elTopButtons = el('project-buttons', [
        this.elShareButton = el('button', 'project-button', [T('Share')]),
        this.elFlipButton = el('button', 'project-button flip', [
          el('i', ''),
          T('See project page')
        ])
      ]),
  ...
  function ScriptsPanel(editor) {
    ...
    this.el = el('scripts-panel', [
      this.elButtons = el('palette-buttons'),
      this.elPalette = el('palette-contents'),
      this.elWorkspace = el('editor-workspace')
    ]);
    this.createButtons();
  ...
  function CostumesPanel(editor) {
    ...
    this.el = el('costumes-panel', [
      this.elNewLabel = el('costume-new-label'),
      this.elNewGroup = el('costume-new-group', [
  ...
  function ImageEditor() {
    ...
    this.elCanvasGrid.style.backgroundColor = '#fff';
    var grad = 'linear-gradient(45deg, #e8e8e8 25%, transparent 25.5%, transparent 74.5%, #e8e8e8 75%)';
    this.elCanvasGrid.style.backgroundImage = grad+', '+grad;
  ...
  function TopBar(editor) {
    ...
    this.el.appendChild(this.elTools = el('top-tools'));
  ...
  TopBar.prototype.addButton = function(text, action, arrow) {
    ...
    if (arrow) {
      button.appendChild(el('arrow'));
    }
    return button;
  };

  TopBar.prototype.addTool = function(name, title) {
    var button = el('button', 'top-tool top-tool-' + name);
    button.title = title;
    this.elTools.appendChild(button);
    return button;
  }
  ...
  TopBar.prototype.showAbout = function() { // NS
    ...
    content.innerHTML = '<article>'+T(
      '<h1>Pixie</h1>'+
      '<p>An unofficial rewrite of <a target=_blank href=https://scratch.mit.edu>Scratch</a> in JavaScript, created by <a target=_blank href=https://github.com/nathan>Nathan Dinsmore</a>.</p>'+
      ...
      '<ul class=links><li><a target=_blank href=https://github.com/nathan/pixie/issues>Issues</a><li><a target=_blank href=https://github.com/nathan/pixie/graphs/contributors>Contributors</a></li>'
    )+'</article>';
  ...
  function SpritePanel(editor) {
    ...
    this.el = el('sprite-panel', [
      this.elTitleBar = el('title-bar', [
        this.elLabel = el('title-label', [T('Sprites')]),
        this.elNewGroup = el('new-group', [
          this.elNewGroupLabel = el('new-group-label', [T('New sprite:')]),
      ...
      this.elStageSection = el('stage-section', [
        this.stageIcon.el,
        this.elNewBackdrop = el('new-backdrop', [T('New backdrop:')])
      ]),
      this.elSpriteSection = el('sprite-section')
    ]);
