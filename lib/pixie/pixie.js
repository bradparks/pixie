(function() {
  ...
  ScratchObj.prototype.setCostume = function(i) {
    ...
    if (costume) {
      ...
      this.pixiSprite.setTexture(null);
    }
  };
  ...
  function Costume(name, canvas, cx, cy, pixelRatio, textLayer) {
    ...
    if (typeof canvas === 'string') { // MD5
      ...
      this.baseLayerTexture = PIXI.Texture.fromImage(Server.getAssetURL(canvas), true, PIXI.scaleModes.NEAREST);
    } else {
      this.baseLayerTexture = PIXI.Texture.fromCanvas(canvas, PIXI.scaleModes.NEAREST);
    }
    if (typeof textLayer === 'string') { // MD5
      this.textLayerMD5 = textLayer;
      this.textLayerTexture = PIXI.Texture.fromImage(Server.getAssetURL(textLayer), true, PIXI.scaleModes.NEAREST);
    }
    this.name = name;
    this.cx = cx || 0;
    this.cy = cy || 0;
  ...
  Sprite.prototype.setPosition = function(x, y) {
    ...
    this.y = y;
    var costume = this.costumes[this.costume];
    this.pixiSprite.position.x = 240 + x - (costume ? costume.cx * costume.scale : 0);
    this.pixiSprite.position.y = 180 - y - (costume ? costume.cy * costume.scale : 0);
  };
  ...
  Sprite.prototype.updateScale = function() {
    var costume = this.costumes[this.costume];
    this.pixiSprite.scale.x =
    this.pixiSprite.scale.y = this.scale * (costume ? costume.scale : 1);
  };
  ...
  Stage.prototype.remove = function(child) {
    ...
    if (child.isSprite) {
      ...
      this.pixiStage.removeChild(child.pixiSprite);
    }
  ...
  Interpreter.prototype.addPrimitives = function(table) {
    ...
    table['nextCostume'] = function() {
      ...
      if (sprite.isSprite && sprite.costumes.length) {
        sprite.setCostume(sprite.costume + 1);
        if (sprite.visible) interp.redraw = true;
      }
    };
    ...
    function startScene(name) {
      ...
      if (i !== -1) interp.stage.setCostume(i);
      interp.redraw = true;
    ...
    table['nextScene'] = function() {
      interp.stage.setCostume(interp.stage.costume + 1);
      interp.redraw = true;
    };
  ...
  CostumesPanel.prototype.select = function(icon) {
    ...
    if (this.selectedIcon = icon) {
      ...
      this.sprite.setCostume(icon.index);
